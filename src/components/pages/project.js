    ...
    createOffChainProject =  async () => {
      ...
      await this.fetchAccount();
       ...
       const creator = JSON.parse(localStorage.getItem('w3ird-pen-artist'))
        ...
        const data =  JSON.stringify({name: name, creator: creator, date: time, description: description, image: preview, html: html, js: js, css: css})
        ...
        .then((response) => {
            ...
            if(this.state.mainHash){
                ...
                this.state.w3irdsTokens.methods.newProject(`https://ipfs.infura.io/ipfs/${ipfsId}`, price).send({ from: this.state.account })
      ...
      let ipfsId
        ...
        .then((response) => {
          ...
          //id++;
            ...
            window.confirm(`Your asset is ready, find it here: https://ipfs.infura.io/ipfs/${ipfsId}` )
            ...
            tR.innerHTML = `<th scope="row">${file.name}</th>
                            <td id="copy" >https://ipfs.infura.io/ipfs/${ipfsId}</td>`
            tB.appendChild(tR)
        ...
        {/*<div className={styles.editorsTitle}>{this.props.projectTitle} {this.props.projectCreator} {this.props.projectTime} </div>*/}
           ...
           >
                    ...
                    <div className="col-md-5 overflow-auto text-center" style={{ marginTop: '10px', maxHeight: '768px', minWidth: '500px' }}>
                        ...
                        <div style={{textAlign: 'left'}}><button className="btn-dark"
                         onClick = { (e) => {
                             e.preventDefault();
                             //this.fetchAccount();
